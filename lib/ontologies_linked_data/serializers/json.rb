    ...
    class JSON
      def self.serialize(obj, options = {})
        hash = obj.to_flex_hash(options) do |hash, obj|
          hash["@id"] = obj.resource_id.value if obj.respond_to?(:resource_id) && !obj.resource_id.bnode?
          hash["@type"] = obj.class.type_uri if hash["@id"] && obj.class.respond_to?(:type_uri)
          context = obj.is_a?(Goo::Base::Resource) ? generate_context(obj) : {}
          hash.merge!(context)
        end
        hash.to_json
      end
      ...
      def self.generate_context(object)
        ...
        class_attributes.each do |attr, settings|
          ...
          if linked_model && !object.resource_id.bnode?
            # linked object
            predicate = {"@id" => linked_model.type_uri, "@type" => "@id"} rescue binding.pry
