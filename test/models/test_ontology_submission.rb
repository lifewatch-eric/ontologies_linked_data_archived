...
class TestOntologySubmission < LinkedData::TestOntologyCommon
  ...
  def test_skos_ontology
    ...
    sub = LinkedData::Models::OntologySubmission.where(ontology: [acronym: "SKOS-TEST"],
                                                     ...
                                                     .include(:version)
                                                     .first
    sub.roots.map { |x| x.id.to_s}.sort == ["http://www.ebi.ac.uk/efo/EFO_0000311",
    ...
    LinkedData::Model::Class.in(sub).models(roots).include(:children).all
    sub.roots.each do |root|
      assert 1 != 0
